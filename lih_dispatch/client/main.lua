...
local function isJobValid(data)
    ...
    elseif type(data) == "table" then
        return lib.table.contains(data, jobName)
    end
...
local function openMenu()
    if not isJobValid(ESX.PlayerData.job.name) then return end

    local calls = lib.callback.await('lih_dispatch:callback:getCalls', false)
    if #calls == 0 then
...
local function setWaypoint()
    ...
    local data = lib.callback.await('lih_dispatch:callback:getLatestDispatch', false)
    ...
    if not waypointCooldown and lib.table.contains(data.jobs, PlayerData.job.name) then
        ...
        --TriggerServerEvent('lih_dispatch:server:attach', data.id, PlayerData)
...
local function createBlip(data, blipData)
    ...
    local sprite = blipData.sprite or (blipData.alert and blipData.alert.sprite) or 161
    local color = blipData.color or (blipData.alert and blipData.alert.color) or 1
    local scale = blipData.scale or (blipData.alert and blipData.alert.scale) or 0.9
    local flash = blipData.flash or false
    local alpha = 255
    local radiusAlpha = 128
    local blipWaitTime = ((blipData.length or (blipData.alert and blipData.alert.length) or 10) * 10000) / radiusAlpha
...
local function addBlip(data, blipData)
    ...
    if not alertsMuted then
        if blipData.sound == "Lose_1st" then
            ...
            exports['lih_sound']:PlayUrl('dispatchsound', './sounds/911.ogg', 0.3, false)
...
-- Keybind
RegisterCommand('openDispatchMenu', function()
    openMenu()
end)
...
-- Events
RegisterNetEvent('lih_dispatch:client:notify', function(data, source)
    if data.alertTime == nil then data.alertTime = Config.AlertTime end
    local timer = data.alertTime * 1000
...
RegisterNetEvent('lih_dispatch:client:openMenu', function(data)
    if not isJobValid(ESX.PlayerData.job.name) then return end
...
RegisterNUICallback("attachSelf", function(data, cb)
    TriggerServerEvent('lih_dispatch:server:attachSelf', data.id, PlayerData)
    if Config.PhoneBridge then
        exports['lb-emergency']:AssignDispatch(data.uuid, PlayerData.name)
    end
    cb("ok")
end)

RegisterNUICallback("detachSelf", function(data, cb)
    TriggerServerEvent('lih_dispatch:server:detachSelf', data.id, PlayerData)
    cb("ok")
end)
...
RegisterNUICallback("showNotify", function(data, cb)
    lib.notify({description = tostring(data), position = 'top', type = 'warning'})
    cb("ok")
end)

if Config.PhoneBridge then
    RegisterNUICallback("removeDispatch", function(data, cb)
        TriggerServerEvent('lih_dispatch:server:remove', data.uuid)
        exports['lb-emergency']:RemoveDispatch(data.uuid)
        DeleteWaypoint()
        cb("ok")
    end)
end 
...
RegisterNUICallback("removeCall", function(data, cb)
    TriggerServerEvent('lih_dispatch:server:removeCall', data.id)
    RemoveBlip(blips[data.id])
    RemoveBlip(radius2[data.id])

    local data = lib.callback.await('lih_dispatch:callback:getCalls', false)
    SendNUIMessage({ action = 'setDispatchs', data = data, })
    cb("ok")
end)
...
RegisterNUICallback("phoneCall", function(data, cb)
    ...
    if inCall then 
        lib.notify({ description = 'Du befindest dich bereits in einem Anruf!', position = 'top', type = 'error' })
    else
        exports["lb-phone"]:CreateCall({number = tostring(data.number)})
    end
    cb("ok")
end)
